...
export default function PedidoDetalhePage() {
  ...
  const { getPedidoById, avancarStatus } = usePedidosStore()
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <InfoCard
            ...
            sub={pedido.cliente?.empresa_nome}
          ...
          <InfoCard
            ...
            bg="bg-amber-50"
            label="Prazo"
            value={format(parseISO(pedido.data_prazo), 'dd/MM/yyyy')}
            sub={atrasado ? '⚠️ Atrasado' : undefined}
            danger={atrasado}
          />
          <InfoCard
            ...
            bg="bg-slate-100"
            label="Entrega"
            value={pedido.tipo_entrega === 'retirada' ? '🏪 Retirada'
              : pedido.tipo_entrega === 'entrega' ? '🚚 Entrega'
              : pedido.tipo_entrega === 'correios' ? '📮 Correios'
              : '—'}
          />
          <InfoCard
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-emerald-600">
                R$ {pedido.valor_pago.toLocaleString('pt-BR')}
              </p>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <div className={`w-2.5 h-2.5 rounded-full ${
                  pedido.status_pagamento === 'pago' ? 'bg-emerald-500'
                  : pedido.status_pagamento === 'parcial' ? 'bg-amber-500'
                  : 'bg-red-400'
                }`} />
          ...
          {pedido.valor_desconto > 0 && (
            <p className="text-xs text-slate-400 mt-3">
              Desconto aplicado: R$ {pedido.valor_desconto.toLocaleString('pt-BR')}
            </p>
          )}
        </div>
        ...
        {(pedido.timeline ?? []).length > 0 && (
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-slate-800 text-sm">Histórico</h3>
            </div>
            <div className="relative">
              <div className="absolute left-3 top-2 bottom-2 w-px bg-slate-200" />
              <div className="space-y-4">
                {[...(pedido.timeline ?? [])].reverse().map((evento) => {
                  const statusCfg = evento.status_novo
                    ? STATUS_CONFIG[evento.status_novo]
                    : null
                  return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {pedido.observacoes && (
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <p className="text-sm text-slate-600 leading-relaxed">{pedido.observacoes}</p>
              </div>
            )}
            {pedido.observacoes_internas && (
              <div className="bg-white rounded-xl border border-amber-200 bg-amber-50 p-5">
                ...
                <p className="text-sm text-amber-800 leading-relaxed">{pedido.observacoes_internas}</p>
              </div>
            )}
          </div>
        )}
...
function InfoCard({ icon, bg, label, value, sub, danger, onClick }: {
  ...
  return (
    <div
      className={`bg-white rounded-xl border border-slate-200 p-4 ${onClick ? 'cursor-pointer hover:border-indigo-200 transition-colors' : ''}`}
      onClick={onClick}
    >
